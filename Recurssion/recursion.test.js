...
describe("findIndex", function () {
  let animals = ["duck", "cat", "puppy", "cat"]
  ...
  it("returns the first index of the value in the array if the value exists", function () {
    ...
    expect(findIndex(animals, "puppy")).toBe(2)
  })

  it("returns -1 if the value does not exist", function () {
    expect(findIndex(animals, "bird")).toBe(-1)
...
describe("revString", function () {
  it("returns a reversed copy of the string", function () {
    expect(revString("dog")).toBe("god")
    expect(revString("duck")).toBe("kcud")
    expect(revString("cat")).toBe("tac")
    expect(revString("puppy")).toBe("yppup")
  })
})
  ...
  it("handles nested objects", function () {
    let nestedObj = {
      ...
      funFacts: {
        ...
        favoriteString: "yesss!"
      }
    }
    expect(gatherStrings(nestedObj).sort()).toEqual(["Bullock", "Cole", "you made it!", "yesss!"].sort())
  })
})
